  ...
  run(async (req, res) => {
    ...
    if (payload.event === "payment.captured") {
      ...
      if (!payment?.id || !payment.order_id)
        throw new HttpError(400, "Invalid payment event");
      ...
      if (booking) {
        ...
        await confirmBooking(booking._id, payment.id);
      }
    }
    ok(res);
  }),
...
app.get("/api/v1/health", (_req, res) =>
  res.status(mongoose.connection.readyState === 1 ? 200 : 503).json({
    ...
    message:
      mongoose.connection.readyState === 1 ? "Ready" : "Database unavailable",
  }),
);
app.post(
  "/api/v1/jobs/run",
  run(async (req, res) => {
    if (
      !env.JOB_SECRET ||
      req.headers.authorization !== `Bearer ${env.JOB_SECRET}`
    )
      throw new HttpError(401, "Unauthorized");
    ok(res, await maintenance());
  }),
);
...
app.use("/api/v1", commerce);
app.use("/api/v1", workspaces);
      ...
      error instanceof ZodError
        ...
        : error.code === 11000
          ...
          : status >= 500 && !(error instanceof HttpError)
            ...
            : error.message;
    if (status >= 500)
      console.error("Request failed:", error.name, error.code || status);
