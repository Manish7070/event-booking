...
const issue = (user: IUser) =>
  jwt.sign(
    { id: user._id.toString(), version: user.tokenVersion },
    env.JWT_SECRET,
    { expiresIn: "8h", issuer: "eventra", audience: "eventra-client" },
  );
auth.use(
  rateLimit({
    windowMs: 15 * 60000,
    ...
    message: {
      success: false,
      error: { message: "Too many authentication requests. Try again later." },
    },
  }),
);
auth.post(
    ...
    ok(res, {
      ...
      organizerProfile:
        req.user!.role === "ORGANIZER"
          ? await OrganizerProfile.findOne({ user: req.user!._id })
          : null,
    }),
  ),
);
...
auth.post(
  ...
  run(async (req, res) => {
    ...
    const user = await User.findOneAndUpdate(
      { resetToken: hash(input.token), resetTokenExpiry: { $gt: new Date() } },
      {
    ...
    ok(res, {}, "Password reset. Sign in with your new password.");
  }),
);
auth.post(
  "/verify-email",
