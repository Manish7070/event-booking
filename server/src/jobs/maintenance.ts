...
export async function maintenance() {
  ...
  for (const booking of pending) {
    if (booking.paymentOrderId) {
      ...
      const payments = await gateway().orders.fetchPayments(
        booking.paymentOrderId,
      );
      const captured = payments.items.find(
        (p) =>
          p.status === "captured" &&
          Number(p.amount) === toPaise(booking.total) &&
          p.currency === "INR",
      );
      if (captured) {
        await confirmBooking(booking._id, captured.id);
        reconciled++;
        continue;
      }
    }
  ...
  const upcoming = await Event.find({
    ...
    startDate: { $gt: new Date(), $lt: new Date(Date.now() + 86400000) },
  }).limit(100);
  for (const event of upcoming) {
    ...
    for (const booking of bookings)
      await notify(
        ...
        "Your event starts within 24 hours. Open My Tickets for your entry passes.",
        `reminder:${booking._id}`,
      );
  }
  ...
  await Ticket.updateMany(
    { event: { $in: ended.map((e) => e._id) }, status: "VALID" },
    { status: "EXPIRED" },
  );
  await Event.updateMany(
    { _id: { $in: ended.map((e) => e._id) } },
    { status: "COMPLETED" },
  );
  ...
  if (env.EMAIL_HOST && env.EMAIL_FROM) {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      try {
        ...
        job.status = "FAILED";
        job.nextAttempt = new Date(Date.now() + 2 ** job.attempts * 60000);
      }
      await job.save();
    }
  }
